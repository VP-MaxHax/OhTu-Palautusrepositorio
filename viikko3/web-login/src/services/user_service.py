...
class UserService:
    ...
    def validate(self, username, password, password_confirmation):
        ...
        if not username.isalpha():
            raise UserInputError("Username must contain only letters from alphabet")
        
        if len(password)<8:
            raise UserInputError("Password must be at least 8 characters long")
        
        if password.isalpha():
            raise UserInputError("Password must contain atleast one number or special character")
